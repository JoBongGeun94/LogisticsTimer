...
// 🔧 메모이제이션 캐시 - 성능 최적화의 핵심
const memoizedCache = new Map<string, any>(); // 키: 해시, 값: 통계 데이터

  // 🔧 디바운스 타이머
  let debounceTimer: NodeJS.Timeout | null = null;
  // 🔧 실시간 통계 분석 (무한 루프 방지)
  const DEFAULT_GAUGE_DATA = {
    grr: 0,
    repeatability: 0,
    reproducibility: 0,
    partVariation: 0,
  ...
  const generateCacheKey = (data: LapTime[]): string => {
    const keyData = data.map(lap => `${lap.operator}:${lap.target}:${lap.time}`).join('|');
  ...
  useMemo(() => {
    ...
    // 🔧 깊은 의존성 추가로 무한 루프 방지
    const dataSignature = lapTimes.map(lap => `${lap.time}-${lap.operator}-${lap.target}`).join(',');
    const cacheKey = generateCacheKey(lapTimes);

    // 🔧 캐시된 결과 확인 (타임스탬프 기반 유효성 검증)
    if (memoizedCache.has(cacheKey)) {
      const cached = memoizedCache.get(cacheKey)!;
      const cacheAge = Date.now() - (cached.timestamp || 0);

      // 캐시 유효 기간: 30초
      if (cacheAge < 30000) {
        console.log(`📋 캐시 히트: ${cacheKey.slice(0, 8)}... (${cacheAge}ms)`);
        return cached;
      } else {
        memoizedCache.delete(cacheKey);
      }
    }
    ...
    try {
      ...
      // 🔧 캐시에 타임스탬프 저장
      memoizedCache.set(cacheKey, { ...atomicResult, timestamp: Date.now() });
      return atomicResult;
      ...
      const batchUpdate = () => {
        ...
        if (allLaps.length >= 2) {
          ...
          if (deltaPairUpdate > threshold && allLaps.length > 2) {
            ...
            // 🔧 타이머 정리를 위한 참조 저장
            (window as any).__retakeModalTimeout = debounceTimer;
          }
        }
